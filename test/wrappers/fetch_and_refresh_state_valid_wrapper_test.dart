...
void main() {
  group('FetchAndRefreshStateValidWrapper test', () {
    ...
    testWidgets('FetchAndRefreshStateValidWrapper test valid state', (WidgetTester tester) async {
      ...
      when(() => personRepository.getObject(idToGet)).thenAnswer(
        (_) async => PersonEntityFixture.factory(idToGet).makeSingle(),
      );
      ...
      expect(
        mockFetchAndRefreshCubit.state,
      ...
      expect(
        mockFetchAndRefreshCubit.state,
        isA<FetchAndRefreshRefreshingSuccessState>(),
      );
      await tester.pump();
      expect(find.byType(ColoredBox), findsOneWidget);
      ...
      expect(
        mockFetchAndRefreshCubit.state,
        isA<FetchAndRefreshInitialState>(),
      );
    ...
    testWidgets('FetchAndRefreshStateValidWrapper test invalid state', (WidgetTester tester) async {
      ...
      expect(
        mockFetchAndRefreshCubit.state,
        isA<FetchAndRefreshFetchingErrorState>(),
      );
      await tester.pump();
